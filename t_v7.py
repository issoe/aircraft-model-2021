...
def combination_function(init__list, status_case, var_indexFlight):
    # If the list is only 1 elements
    if len(init__list) == 1:
        # print("test:", init__list[0][0], init__list[0][1], init__list[0][2], init__list[0][3], init__list[0][4])
        ...
        if status_case == 1:
            ...
            if len(df[1][var_indexFlight]) == 4:
                ...
                print("Get len df [1][index] fault at:", )      # <FIX> -- maybe delete this line
            ...
            # 1. Find a suitable position
            pos, level, init_t, final_l = find_terminal(df[1][indexFlight][0:4], arr_codeFlights[indexFlight][0], -1, arr_timeDeparture[indexFlight], str(arr_last_colum[indexFlight]))
            ...
            if level == 0:
                ...
                time_A = level
            time_D = arr_timeDeparture[var_indexFlight] 
            ...
            print("waiting case 4 - arr_waiting here")

    
    # The list have a little flight that keep the same time_Departure
    else:
        #########################################################################################
        # COMBINATION PROCESS HERE ############################################################## 
        idxx = first = second = 0
        temp_list = init__list
        # minValueOfAllObjectiveFunction = 4
        while(idxx < ncr(len(init__list), 2)):
            second += 1
            temp_val = temp_list[first]
    ...
    try:
        ...
        print("Fault at clearing the list in combination function")
...
for time_minute in range(0, 1440):
    while (time_minute == arr_timeDeparture[indexFlight]) or (arr_timeDeparture[indexFlight] == -1):
        ...
        if arr_schedule[indexFlight] == '1':
            # 1. Find a suitable position            
            pos, level, init_t, final_l = find_terminal(df[1][indexFlight][0:4], arr_codeFlights[indexFlight][0], arr_timeArrival[indexFlight], arr_timeDeparture[indexFlight], str(arr_last_colum[indexFlight]))
            
            # 2. Add the flight to the result
            arr_result.append([indexFlight, df[1][indexFlight][0:4], arr_codeFlights[indexFlight][0], arr_timeArrival[indexFlight], arr_timeDeparture[indexFlight], pos, df[1][indexFlight][0:4], arr_codeFlights[indexFlight][1], init_t, final_l])
        ...
        if arr_schedule[indexFlight] == '2':
            ...
            # 4. Adding the result_array
            arr_result.append([indexFlight, kind_Light_Arrival, flightNo_Arrival, time_A, time_D, pos, df[1][indexFlight][0:4], arr_codeFlights[indexFlight][0], init_t, final_l])
