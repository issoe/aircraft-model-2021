...
def find_terminal(kindOfFlight, isVN, time_A, time_D, last_colum): # print("\nFind terminal:", kindOfFlight, isVN, time_A, time_D, last_colum)    
        ...
        if isVN[0:2] == "VN":
            for idx in arr_priority_1: # idx = position 
                ...
                if (temp_objective < min_objective) and ((temp_level + time_break) < var_time):
                    ...
                    # only get ONE time                         #  <ONE time>
                    if initial_value_Ojective == 4:             #  <ONE time>
                        initial_value_Ojective = min_objective  #  <ONE time>
...
def combination_function(init__list, status_case, var_indexFlight):
    if len(init__list) == 1:
        if status_case == 1:
            ...
            pos, level, init_t, final_l = find_terminal(init__list[0][0], init__list[0][1], init__list[0][2], init__list[0][3], init__list[0][4])

            # 2. Update again level with time_Departure
            newlevel = arr_timeDeparture[var_indexFlight]
            update_level(pos, newlevel, df[1][var_indexFlight][0:4], arr_codeFlights[var_indexFlight][0]) 

            # 3. Add the flight to the result
            arr_result.append([var_indexFlight, df[1][var_indexFlight][0:4], arr_codeFlights[var_indexFlight][0], arr_timeArrival[var_indexFlight], arr_timeDeparture[var_indexFlight], pos, df[1][var_indexFlight][0:4], arr_codeFlights[var_indexFlight][1], init_t, final_l])
            ...
            arr_result.append([var_indexFlight, kind_Light_Arrival, flightNo_Arrival, time_A, time_D, pos, df[1][var_indexFlight][0:4], arr_codeFlights[var_indexFlight][0], init_t, final_l])
            ...
            # 1. Find a suitable position
            pos, level, init_t, final_l = find_terminal(df[1][var_indexFlight][0:4], arr_codeFlights[var_indexFlight][0], -1, arr_timeDeparture[var_indexFlight], str(arr_last_colum[var_indexFlight]))
        ...
        if status_case == 1:
            ...
            # MEANING: each loop will be a COMBINATION -> If minValueOfAllObjectiveFunction minimum -> chose this case
            while(idxx < ncr(len(init__list), 2)):
                second += 1
                temp_val = temp_list[first]    # temp variables for swap 2 values 
                temp_list[first] = temp_list[second]
                temp_list[second] = temp_val
                # CALCULATE DATA  #################################################################################
                # INITIAL VARIABLES AND ARRAY FOR EACH COMBINATION
                # After each loop, this value will be res
                temp_value_minimum = 0  

                arr_temp_case_optimise = []
                arr_temp_pos = []

                for insideLoop in range(0, len(init__list)):
                    # 1. Find a suitable position            
                    pos, level, init_t, final_l = find_terminal(temp_list[insideLoop][0], temp_list[insideLoop][1], temp_list[insideLoop][2], temp_list[insideLoop][3], temp_list[insideLoop][4])

                    # 2. Increase temp_value_minimum for finding optimise method
                    temp_value_minimum += final_l

                    # 3. Add this flight into case_optimise array and saving all the pos into temp_array_avoid_pos array
                    arr_temp_case_optimise.append([temp_list[insideLoop][0], temp_list[insideLoop][1], temp_list[insideLoop][2], temp_list[insideLoop][3], temp_list[insideLoop][4]])
                    arr_temp_pos.append(pos)
                


                # Definitely get the first case COMBINATION
                # If new temp_value_minimum more optimise -> CHANGE HERE
                if temp_value_minimum < minValueOfAllObjectiveFunction:
                    minValueOfAllObjectiveFunction = temp_value_minimum
                    arr_case_optimise = arr_temp_case_optimise
                    arr_pos_optimise = arr_temp_pos
                    print("count here")

                # Next combination 
                arr_temp_case_optimise.clear()
                arr_temp_pos.clear()

                # NEXT COMBINATION  ###################################################################################
                if second == (len(init__list) - 1):    
                    first += 1
                    second = first
                idxx += 1
            
            # Add case_optimise into pos_optimise

            # # Found the optimise case by using combination algorithm -> ADD, wait???
            # for adder_pos in arr_pos_optimise:
            #     # 1. Update again level with time_Departure
            #     newlevel = arr_timeDeparture[temp_var_indexFlight]
            #     update_level(adder_pos, newlevel, df[1][temp_var_indexFlight][0:4], arr_codeFlights[temp_var_indexFlight][0]) 

            #     # 2. Add the flight to the result
            #     arr_result.append([temp_var_indexFlight, df[1][temp_var_indexFlight][0:4], arr_codeFlights[temp_var_indexFlight][0], arr_timeArrival[temp_var_indexFlight], arr_timeDeparture[temp_var_indexFlight], adder_pos, df[1][temp_var_indexFlight][0:4], arr_codeFlights[temp_var_indexFlight][1], init_t, final_l])
...
for time_minute in range(0, 1440):
    while (time_minute == arr_timeDeparture[indexFlight]) or (arr_timeDeparture[indexFlight] == -1):
        # case 1: ?-SGN-? ---------------------------------------------------------------------------------------------------------------
        # while (time_minute == arr_timeDeparture[indexFlight]) and (arr_schedule[indexFlight] == '1'):
        if arr_schedule[indexFlight] == '1':
            # 1. Add flight into arr_tempMission and TURN ON "modeOne"
        ...
        # case 1.2. Call the combination function and turn off the status modeOnde (genetic algorithm)
        if modeOne == True:
        ...
        # case 2.2. Call the combination function
        if modeTwo == True:
        ...
        # case 3.2. Call the combination function and turn off the status modeThree
        if modeThree == True:
    ...
    try:
        while time_minute == arr_waiting_flight[0][0]:  
            # 1. Find a suitable position
            pos, level, init_t, final_l = find_terminal(arr_waiting_flight[0][2], arr_waiting_flight[0][1], -1, arr_waiting_flight[0][0], arr_waiting_flight[0][3])
